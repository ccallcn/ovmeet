...
public class ChatServer24 extends ApplicationAdapter
{
	...
	public ChatServer24()
	{
		...
		*/
		//System.out.println((new StringBuilder("你现在服务器License人数为:")).append(appMaxUser).toString());
		
		
		
		//if (MemThread.hasBuy)
		//	System.out.println("License是正式版本!");
		//else
		//	System.out.println("License是试用版本!");
	}
	...
	public boolean appStart(IScope iscope)
	{
		...
		//log.info((new StringBuilder("视频会议服务器 start-")).append(appMaxUser).append(" concurrence user version").toString());
		return true;
	}
	...
	public boolean roomStart(IScope iscope)
	{
		try
		{
			...
			isharedobject4.setAttribute("g_ds", Boolean.valueOf(true));//听众禁止请求发言
			isharedobject4.setAttribute("g_dp", Boolean.valueOf(false));//听众禁止私聊
			isharedobject4.setAttribute("g_dt", Boolean.valueOf(false));//听众禁止文本发言
			isharedobject4.setAttribute("a_dv", Boolean.valueOf(false));//禁止所有视频
			isharedobject4.setAttribute("a_da", Boolean.valueOf(false));//禁止所有音频
	...
	public boolean roomConnect(IConnection iconnection, Object aobj[])
	{
		...
		*/
		...
		/**取参数长度大于0  aobj.length<1，是不是管理员 j!=2 标志2为管理员，　房间锁定标志flag,　MemThread.isExpired()授权认证是不是到期
		...
		**/
		...
		//System.out.println("参数个数："+aobj.length+"锁定标志："+flag+"用户级别："+j+"用户名："+userName);
		...
		//	break MISSING_BLOCK_LABEL_608;
			...
			{ 
				...
				//更新界面用户连接的用户ＩＤ号，ＩＤ号是服务端自动生成的，这里调用返回给客户端
				...
				iservicecapableconnection.invoke("updateRoomInfo", new Object[] {
						//房间最大人数MemThread.maxUser，是否已购买MemThread.hasBuy，true购买，flase非购，显示信息：MemThread.companyName
				...
				/**授权通过调用flash客户端 showTryWin方法，弹出欢迎界面 MemThread.isTria试用标志,true试用，flase非试用
				if (MemThread.isTria)
				{
					String strWelcome = "欢迎试用CeCallMeet,网址http://www.cecall.cc!";
					iservicecapableconnection.invoke("showTryWin", new Object[] {
						strWelcome
					});
				}
				**/
				//System.out.println("查看用户的配置信息！");
				
				if (userList2.getAttribute(userName) != null)
				{
					String s2 = (String)userList2.getAttribute(userName);
					kickUser(s2);//踢除上一次的会议登录
