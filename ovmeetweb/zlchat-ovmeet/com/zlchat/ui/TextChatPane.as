...
package com.zlchat.ui {
    ...
    public class TextChatPane extends BasePane {
        ...
        protected function sendMsg(_arg1:Event):void{
            if (txtMsgSend.text != ""){
               // nc.netGroup.post(new String(((((((((("<font size='+1' color='#ff6633'> " + nc.user.realName) + "(") + ChatConnection.getChineseTime()) + "):</font>") + "<font size='+2' color='") + txtColor) + "'> ") + txtMsgSend.text) + "</font><br>")));
				var chatMsg:String = (new String(((((((((("<font size='+1' color='#ff6633'> " + nc.user.realName) + "(") + ChatConnection.getChineseTime()) + "):</font>") + "<font size='+2' color='") + txtColor) + "'> ") + txtMsgSend.text) + "</font><br>")));
				nc.netGroup.post(chatMsg);
				sendMessage(chatMsg);
                txtMsgSend.text = "";
                txtMsgSend.setFocus();
            };
        ...
        override public function set conn(_arg1:ChatConnection){
			...
			nc.netGroup.addEventListener(NetStatusEvent.NET_STATUS, netGroupStatusHandler);//监听接收消息
            textChatSo = SharedObject.getRemote("textChat", nc.uri, false);//生成共享对像
            textChatSo.connect(nc);//共享对像连在连接上
            textChatSo.client = this;//共享对像接一个客户端,到本类
        }
		//监听接收消息
        private function netGroupStatusHandler(evt:NetStatusEvent) : void
        { 
			//logMessage("连接信息: " + evt.info.code);
			//trace("textChat"+evt.info.code);
			switch(evt.info.code) 
			{
				case "NetGroup.Posting.Notify":
						sendMessage(evt.info.message);
					break; 
        ...
        public function sendMessage(_arg1:String):void{
			//服务器textChat共享对像回调的方法 或rtmfp监听消息的方法
            var _local2:Msg;
